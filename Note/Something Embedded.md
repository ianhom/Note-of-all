...
```
一般的队列都有头结点和尾结点，头结点是最早进入队列的数据，尾结点是下一个数据即将放入的结点，所以通过尾结点减去头结点就可以知道有多少个。当然这里还有一个例外，就是头尾结点相同时，可以表示队列为空，或表示队列为满。对了处理这样的误解，我们在队列中本身就增加一个计数器用于统计目前有多少个有效数据。而头结点、尾结点和如队列的计数三者的关系就是只其二可的第三，所以这里设计只有头结点和计数，尾结点通过头结点+计数可获得。
