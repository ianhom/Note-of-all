...
- 库的限制 —— 宏定义，因为库是源文件经过预处理、编译和汇编之后的产物，所以宏定义在预处理阶段就已经转换成了具体的替代物，这样即使外部改变了宏定义的值，也不会改变库中的定义。
...
- 在没有MMU（内存管理单元）的平台上，频繁的使用堆会造成碎片，例如申请了10个32字节的内存块，但间隔释放掉5个，此时虽然有5\*32字节的空间，但仍然无法申请一个64字节的内存块出来，因为堆中内存都是碎片化存在的。
...
- 从另一个角度来讲，链表的概念在操作系统中是非常基础的抽象元素，对其充分理解有助于理解更高层的概念抽象，例如定时器、消息等。
...
## 5.1 模块化
- 作为嵌入式软件工程师，大部分时间相处的还是面向过程的C语言，但随着代码越来越复杂，需要描述的过程也会越来越多，若组织不好，工程师对代码的掌控就会渐渐变得困难，步入失控。这时就需要更合理的组织方式来编程。
- 首先我们来看看开发场景，很多嵌入式软件工程师都是一人独自负责一个项目，这样的安排在创业型公司很常见。这是嵌入式软件工程师基本上要负责其产品的全部底层和应用代码（甚至是硬件设计），在人力不发生变化的情况下，这样开发模式暂看不出什么问题，但是随着项目增多，这位开发人员会编写很多代码，其中有很多时重复的，反复的重新显然是浪费的时间的，所以简单的方式就把原来的代码复制一份，然后修改。比如工程师甲开发产品A和产品B。。。。
- **To be continue**
...
## 算法
- 人与人不同体现在思想，技术问题解决的好坏在于算法。好的算法可以帮助计算机高效完成任务，甚至可以讲不可行的计算转变为可操作的运算。掌握算法不是炫技，而是磨炼自己的思维。
- 算法相关分类有排序，搜索，求最优解等
- 排序有冒泡排序，快速排序，睡眠排序等等，每种排序一般会分析时间和空间复杂度，即关心排序所需要消耗的时间和空间。
## 5.5 ...
- **To be continue**
...
### 6.2 关于学习
- 工程师是这样一种职业：将知识转换成实物，知识是原材料，而获取原材料的方法就是学习。
- 学习是自己的事
- 很多工程师在择业的时候，都期望企业为个人能力提供支持与推动。但其实企业提供的不是直接的助力，而是机会，是否能把握机会并等到提升，关键还在自己的学习成长。说句绝情的话，企业与员工就是雇佣关系，付钱请你完成工作，至于你能不能完成这是在面试时就双方认定的事，企业没有义务把个人提升当做重点，所以即使个人有很好的机会去不错的平台，也要认清一个现实，那就是学习是自己的事。    
- 学习需要坚持
- 学习需要反馈
- 在各个行业发展的早期，解决从无到有是首要问题，技术也是一样，如果有了技术经验，那对于没有接触过该技术，没有条件积累经验的人来说，就有很大的难度去改变处境。但多年前信息爆发，导致了现在开发资源越来越容易获得，拥有学习资料和经验变得异常轻松，这样就不能再靠有无来体现价值。那在知识爆棚的时代，靠什么胜出呢，靠谁学的快。

# 杂记
- 最近从事了和嵌入式不是那么相关的业务，从另一个视角重新了解了嵌入式，有很多新感受。
