...
- rtos创建任务时可从heap中分配stack，也可以在全局静态区设定一段内存作为栈空间。
```c
...
#### 全局区
- c语言中定义的顺序不一定是内存空间的排放顺序，编译器将数据的值和size，还有对齐等要素来安排。
- 非零全局变量的初值是通过ROM的数据保存的。其变量排序在RAM和ROM区内相同。在初始化时自动将ROM数据整体搬移到RAM区域。
- 为零全局变量的初值则是通过整体赋值0来初始化。
- 存放哎全局区的数据生命周期是全周期的，意味着到系统掉电之前，数据都时候有效的。
...
- Cpp在c语言的基础之上增加了许多现在高级语言具备的特性，可以说很多高级语言的特性，也是在不断使用c的过程中，一步步发展而来：
    - 继承，一个子类可以继承父类的非private成员和操作，通过该特性可以用很简单的基类构造复杂的类型。。
    - 多态，子类可以重写父类的某些功能以实现定制化需求，通过虚函数实现，纯虚函数是纯接口存在，必须有实例重写。。
    - 函数重载，同一个函数，通过参数的不同来执行不同的操作。
    - 操作符重载，可以重新定义某些操作符的操作方法，比如加号为两个字符串衔接。
- cpp支持模板以实现泛化编程，相同的操作，针对不同的类型或类。
- 友远函数可以实现private成员的访问，但这将降低类的封装特性。
- 对象的接口是开放出来用于外部访问。
- 因为cpp函数编译时会在函数名上增加前后缀，所以当某个cpp函数需要被c程序调用时，需要通过extern "C"来声明cpp函数。
...
- 变量可以通过赋值的类型自动推导变量的类型，例如`i := 10`则i为int类型
- `;`分号不是每行必须的，但是如果一行代码中有多条语句，需要增加分号。编译器会自动在行末增加分号
...
- git checkout 切换分支，也可以用于撤销某个文件的修改
   ...
   - `git checkout -b nnn` 创建并切换到nnn分支
...
- 针对不同的应用，需要权衡空间和时间的关系，可以适时进行空间和时间资源置换，就是常说的空间换时间，时间换空间。
### 哈希
- 哈希是一种大数映射成小数的方法。
